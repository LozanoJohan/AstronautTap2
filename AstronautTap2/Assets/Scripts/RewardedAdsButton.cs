    ...
    public void OnUnityAdsDidFinish (string placementId, ShowResult showResult) 
    {
        ...
        if (showResult == ShowResult.Finished) {
            if (placementId == "Rewarded_Android") {
                ...
                StaticStuff.player.transform.position = new Vector2( 0f, camara.transform.position.y - 2);
                StaticStuff.player.SetActive(true);
                StaticStuff.player.GetComponent<Rigidbody2D>().velocity = Vector2.zero;

                canvas.GetComponent<Animator>().SetTrigger("Close");
                canvas1.SetActive(true);
                StaticStuff.audioSource.mute = false;
            }
            else 

...
public class Buy : MonoBehaviour
{
    ...
    {
        PlayerPrefs.SetString(character, "HasCharacter");
    ...
    public void BuySkin(int index)
    {
        ...
        PlayerPrefs.SetString(character, "HasSkin");
    ...
    void SetDefaultValues()
    {
        ...
        if (PlayerPrefs.GetInt(character+"upg", 0) > 7) upgradeButton.interactable = false;
        if (PlayerPrefs.GetString(character) == "HasSkin")  buySkinBtn.gameObject.SetActive(false);

        for (int i = 0; i < PlayerPrefs.GetInt(character+"upg", 0); i++)
        {
        ...
        {
            case "1": 
                secondsUpg.text = $"<color=green>{50f + PlayerPrefs.GetInt(character+"upg", 0) * 6f}%</color> less pullback";
                break;
            case "2": 
                secondsUpg.text = $"stars have <color=green>{7 + PlayerPrefs.GetInt(character+"upg", 0)*5}%</color> chance to worth double";
                break;
            case "3": 
                secondsUpg.text = $"you become invincible each <color=green>{50 - PlayerPrefs.GetInt(character+"upg", 0)*3}</color> stars";
                break;
        }
        SelectCharacter(PlayerPrefs.GetInt("PersonajeSeleccionado", 0));
    }
    ...
    void SelectCharacter(int index)
    {
        ...
        SetCharacter(Characters.characters[index]);
    }

    void UpgradeCharacter()
    {
        if (PlayerPrefs.GetInt(character+"upg", 0) + 1 > 7) upgradeButton.interactable = false;
        else upgradePrice.text = Mathf.RoundToInt(int.Parse(upgradePrice.text) * multipier).ToString();

        multipier -= minusFactor;
        PlayerPrefs.SetInt(character+"upg", PlayerPrefs.GetInt(character+"upg", 0) + 1);

        switch (character)
        {
            case "1": 
                float pj1 = 50f + (PlayerPrefs.GetInt(character+"upg", 0)) * 6f;
                secondsUpg.text = $"<color=green>{pj1}%</color> less pullback";
                PlayerPrefs.SetFloat("Poder1", pj1);
                break;
            case "2": 
                float pj2 = 7 + (PlayerPrefs.GetInt(character+"upg", 0))*5;
                secondsUpg.text = $"stars have <color=green>{7 + (PlayerPrefs.GetInt(character+"upg", 0))*5}%</color> chance to worth double";
                PlayerPrefs.SetFloat("Poder2", pj2);
                break;
            case "3": 
                float pj3 = 50 - (PlayerPrefs.GetInt(character+"upg", 0))*3;
